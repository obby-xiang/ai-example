<template>
  <div>
    ...
    <el-dialog v-model="showPreview" width="920px" :title="`预览 - ${previewRow?.name || ''} 前100行`">
      <div v-if="previewRows.length" style="overflow:auto;max-height:480px;">
        <el-table :data="previewRows" size="small" border max-height="460">
          <el-table-column type="index" label="#" width="50" />
          <el-table-column
            v-for="(c, i) in previewColumns"
            :key="i"
            :label="c.label"
            :prop="'rowData.'+c.key"
            min-width="120"
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="downloadDef(previewRow)">下载此配置JSON</el-button>
      </template>
    </el-dialog>
  </div>
</template>
