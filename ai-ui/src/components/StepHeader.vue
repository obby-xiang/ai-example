  ...
  <div class="step-indicator">
    <div class="page-card" style="padding:16px 20px;margin-bottom:20px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
        ...
        <div>
          <span style="font-size:12px;color:#909399;">步骤 {{ currentIndex + 1 }} / {{ steps.length }}：</span>
      ...
      <el-steps :active="currentIndex" finish-status="success" simple :process-status="'process'">
        <el-step v-for="(s, i) in steps" :key="s" :title="stepLabelMap[s]" :status="i < currentIndex ? 'success' : (i === currentIndex ? 'process' : 'wait')" />
      </el-steps>
    </div>
  </div>
</template>
...
const currentIndex = computed(() => {
  const i = steps.value.indexOf(currentStep.value)
  return i < 0 ? 0 : i
})
</script>
