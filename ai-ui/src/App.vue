  ...
  <div class="app-container">
    <div class="app-header">
      ...
      <div class="meta">
        ...
        <el-tag type="warning" size="small">JDK21 + Spring Boot 3.5.14</el-tag>
      </div>
    </div>
...
/**
 * 当任务的 currentStep 变化时,自动跳转对应路由(由AI或向导触发)
 */
watch(
...
onMounted(async () => {
  try {
    ...
    // 如果当前有任务,加载AI历史
    if (taskStore.currentTaskId) {
      await aiStore.loadHistory(taskStore.currentTaskId)
      // 如果还没消息,触发一次自动提示
...
function collectWorkspaceState(event) {
  ...
  return {
    ...
    definitions: configStore.enabledDefinitions.map(d => ({
